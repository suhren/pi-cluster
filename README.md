...
subnet 192.168.127.0 netmask 255.255.255.0 {
    interface enp0s31f6;
    range 192.168.127.110 192.168.127.200;
...
The devices should now be assigned an IP address which can be listed with the command. Note that this only lists the dynamically allocated IPs. If we have specified static IPs they will not show up here.
...
jupyter notebook --ip <LAN_ip> --port 8888
```